...
.PHONY: all
all: diag_mkl diag_mkl_omp diag_magma diag_magma_gpu

diag_mkl: diag_f.F90
        bash -l -c "module load compilers/ifort/latest && ifort -DUSE_MKL -O3 $< -o $@ -lmkl_core -lmkl_intel_lp64 -lmkl_sequential"

diag_mkl_omp: diag_f.F90
        bash -l -c "module load compilers/ifort/latest && ifort -DUSE_MKL -O3 $< -o $@ -lmkl_intel_thread -lmkl_core -lmkl_intel_lp64 -liomp5 -lpthread -lm -ldl"

diag_magma: diag_f.F90
        bash -l -c 'module load compilers/ifort/latest && ifort -DUSE_MAGMA $(MAGMA_F90FLAGS) -O3 $< -o $@ $(MAGMA_LIBS)'

diag_magma_gpu: diag_f.F90
        bash -l -c 'module load compilers/ifort/latest && ifort -DUSE_MAGMA_GPU $(MAGMA_F90FLAGS) -O3 $< -o $@ $(MAGMA_LIBS)'
